...
class TestRegisterRoute:
    ...
    @pytest.mark.parametrize("payload, error_field, error_message", [
        # --- 必須フィールド欠落 ---
        ({"email": "a@b.com", "password": "Password123"}, "username", "Missing data for required field."),
        ({"username": "test", "password": "Password123"}, "email", "Missing data for required field."),
        ({"username": "test", "email": "a@b.com"}, "password", "Missing data for required field."),
        
        # --- 空の文字列やフォーマット違反 ---
        ({"username": "", "email": "a@b.com", "password": "Password123"}, "username", "ユーザー名は必須です。"),
        ({"username": "test", "email": "invalid-email", "password": "Password123"}, "email", "有効なメールアドレスを入力してください。"),
        
        # --- パスワード強度違反 ---
        ({"username": "test", "email": "a@b.com", "password": "short"}, "password", "パスワードは8文字以上で入力してください。"),
        ({"username": "test", "email": "a@b.com", "password": "password123"}, "password", "パスワードには少なくとも1つの大文字が必要です。"),
        ({"username": "test", "email": "a@b.com", "password": "PASSWORD123"}, "password", "パスワードには少なくとも1つの小文字が必要です。"),
        ({"username": "test", "email": "a@b.com", "password": "PasswordAbc"}, "password", "パスワードには少なくとも1つの数字が必要です。"),
    ])
